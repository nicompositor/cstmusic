(() => {
  ...
  if (localStorage.getItem("st-calendar-version") !== calendarVersion) {
    ...
    const institutionalFields = ["date", "endDate", "bimester", "title", "type", "scope", "gradeIds", "noClass"];
    const defaults = (data.meta.schoolCalendar || []).map((item) => {
      const merged = { ...item, ...(savedById.get(item.id) || {}) };
      institutionalFields.forEach((field) => {
    ...
    const retiredIds = new Set(data.meta.calendarRetiredIds || []);
    state.calendarPlan = [...defaults, ...state.calendarPlan.filter((item) => !defaultIds.has(item.id) && !retiredIds.has(item.id))];
  ...
  const activityById = new Map((data.meta.activityBank || []).map((activity) => [activity.id, activity]));
  const activityCategoryById = new Map((data.meta.activityCategories || []).map((category) => [category.id, category]));

  function lessonOrdinal(bimester, week, lessonIndex) {
    const weekIndex = Math.max(0, bimester.weeks.indexOf(week));
    return bimester.weeks.slice(0, weekIndex).reduce((total, item) => total + item.lessons.length, 0) + lessonIndex;
  }

  function activityOptionsFor(grade, bimester, week, lessonIndex) {
    if (grade.stage !== "preescolar") return [];
    const eligible = (data.meta.activityBank || []).filter((activity) =>
      activity.enabled !== false
      && activity.levels.includes(grade.id)
      && activity.bimesters.includes(bimester.number));
    if (!eligible.length) return [];
    const gradeOffset = { prejardin: 0, jardin: 5, transicion: 11 }[grade.id] || 0;
    const ordinal = lessonOrdinal(bimester, week, lessonIndex) + gradeOffset;
    const pools = [
      eligible.filter((activity) => ["attention", "warmup", "kodaly"].includes(activity.category) && activity.space !== "Exterior"),
      eligible.filter((activity) => ["warmup", "movement", "kodaly"].includes(activity.category) && ["Exterior", "Espacio amplio", "Flexible"].includes(activity.space)),
      eligible.filter((activity) => ["creation", "visual", "kodaly"].includes(activity.category)),
    ];
    const labels = ["A · Salón y confianza", "B · Movimiento", "C · Creación o instrumentos"];
    const used = new Set();
    return pools.map((pool, index) => {
      const source = pool.length ? pool : eligible;
      let activity = source[(ordinal + index * 7) % source.length];
      for (let tries = 0; tries < source.length && used.has(activity.id); tries += 1) {
        activity = source[(ordinal + index * 7 + tries + 1) % source.length];
      }
      used.add(activity.id);
      return { ...activity, optionLabel: labels[index] };
    });
  }

  const selectedActivityId = (key) => localStorage.getItem(`st-activity-${key}`) || "";

  function activityNamesForLesson(grade, bimester, week, lessonIndex) {
    return activityOptionsFor(grade, bimester, week, lessonIndex).map((activity) => activity.title).join(" · ");
  }

  function renderActivityChoices(grade, bimester, week, lessonIndex, key, interactive = true) {
    const options = activityOptionsFor(grade, bimester, week, lessonIndex);
    if (!options.length) return "";
    const selected = selectedActivityId(key);
    return `<section class="activity-choice-panel full">
      <div class="activity-choice-heading">
        <div><span class="label">Juego del día · elige uno</span><h3>Tres opciones para el mismo encuentro</h3></div>
        <a class="text-button no-print" href="#actividades">Abrir banco completo →</a>
      </div>
      <div class="activity-choice-grid">
        ${options.map((activity) => `<article class="activity-choice ${selected === activity.id ? "selected" : ""}">
          <span class="activity-option-label">${escapeHtml(activity.optionLabel)}</span>
          <h4>${escapeHtml(activity.title)}</h4>
          <p>${escapeHtml(activity.summary)}</p>
          <div class="activity-choice-meta"><span>${escapeHtml(activity.space)}</span><span>${escapeHtml(activityCategoryById.get(activity.category)?.title || "Actividad")}</span></div>
          ${interactive ? `<button class="small-button ${selected === activity.id ? "secondary" : ""}" data-action="choose-activity" data-key="${escapeHtml(key)}" data-activity="${escapeHtml(activity.id)}">${selected === activity.id ? "Elegida ✓" : "Elegir esta"}</button>` : ""}
        </article>`).join("")}
      </div>
      <p class="muted activity-choice-note">Se realiza solamente una opción. Puedes decidir según el espacio, la energía del grupo y los materiales disponibles.</p>
    </section>`;
  }
  ...
  function describeCurriculumTag(tag) {
    ...
    const labels = {
      ...
      guidingQuestion: "Pregunta guía", themeTopics: "Temas del bimestre", themeConnection: "Conexión con el tema",
  ...
  function isNoClassDate(iso, grade) {
    return eventsOnDate(iso).some((event) => event.noClass && calendarScopeMatches(event, grade));
  }
  ...
  function groupSessionDates(group, grade, bimesterNumber) {
    ...
    while (cursor <= end) {
      ...
      if (meetingDays.has(weekdayFromIso(iso)) && !isNoClassDate(iso, grade)) result.push(iso);
        ...
        <section class="hero">
          <div class="hero-copy">
            ...
            <div class="button-row">
              ...
              <a class="button secondary" href="#actividades">Elegir actividades</a>
  ...
  function resolveScheduledClass(entry) {
    ...
    if (state.calendarAuto && !period) return null;
      ...
      <details class="daily-class-card ${grade.stage === "preescolar" ? "preschool" : ""}">
        ...
        <div class="daily-class-main daily-class-expanded">
          ...
          <div class="daily-details">
            ...
            ${lesson.themeConnection ? `<section class="daily-theme"><span class="label">Tema transversal</span><p><strong>${escapeHtml(bimester.title)}</strong></p><p>${escapeHtml(lesson.themeConnection)}</p></section>` : ""}
          ...
          ${renderActivityChoices(grade, bimester, week, lessonIndex, key)}
            ...
            (lesson, index) => `
              <div class="lesson-card-body">
                <div>
                  ...
                  ${grade.stage === "preescolar" ? `<p class="lesson-activity-options"><strong>Elige un juego:</strong> ${escapeHtml(activityNamesForLesson(grade, bimester, week, index))}</p>` : ""}
                </div>
  ...
  function renderBimester(grade, bimester) {
    ...
    return `
      <section class="bimester-hero">
        ...
        ${bimester.guidingQuestion ? `<div class="theme-route"><div><span class="label">Pregunta guía</span><strong>${escapeHtml(bimester.guidingQuestion)}</strong></div><div><span class="label">Temática presente durante todo el bimestre</span>${list(bimester.themeTopics || [])}</div></div>` : ""}
  ...
  function activityLevelLabel(id) {
    return ({ prejardin: "Prejardín", jardin: "Jardín", transicion: "Transición" })[id] || id;
  }

  function renderActivities() {
    setActiveNav("actividades");
    const categories = data.meta.activityCategories || [];
    const activities = data.meta.activityBank || [];
    app.innerHTML = `
      <div class="page">
        <header class="page-header activities-header">
          <div>
            <span class="eyebrow">Banco anual editable desde la planeación</span>
            <h1>Actividades musicales</h1>
            <p>${activities.length} propuestas organizadas por intención. En cada clase de Preescolar aparecen tres opciones; aquí puedes revisar el banco completo y sus adaptaciones para Primaria.</p>
          </div>
          <a class="button secondary" href="#curriculo">Volver al currículo</a>
        </header>

        <section class="activity-principles panel">
          <article><strong>1</strong><span>Elige solo una actividad por clase.</span></article>
          <article><strong>2</strong><span>La repetición aumenta dificultad, liderazgo o creación.</span></article>
          <article><strong>3</strong><span>Exterior, contacto y eliminación siempre se adaptan con seguridad.</span></article>
        </section>

        <section class="filters activity-filters" aria-label="Filtrar actividades">
          <div class="field activity-search"><label for="activity-search">Buscar</label><input id="activity-search" type="search" value="${escapeHtml(state.activitySearch)}" placeholder="Nombre, descripción o habilidad…" /></div>
          <div class="field"><label for="activity-category">Categoría</label><select id="activity-category"><option value="all">Todas</option>${categories.map((category) => `<option value="${category.id}" ${state.activityCategory === category.id ? "selected" : ""}>${escapeHtml(category.title)}</option>`).join("")}</select></div>
          <div class="field"><label for="activity-level">Curso recomendado</label><select id="activity-level"><option value="all">Todo Preescolar</option>${["prejardin", "jardin", "transicion"].map((level) => `<option value="${level}" ${state.activityLevel === level ? "selected" : ""}>${activityLevelLabel(level)}</option>`).join("")}</select></div>
        </section>

        <div class="activity-groups" id="activity-groups">
          ${categories.map((category) => {
            const items = activities.filter((activity) => activity.category === category.id);
            return `<section class="activity-group" data-activity-group="${category.id}">
              <div class="section-heading"><div><span class="eyebrow">${items.length} propuestas</span><h2>${escapeHtml(category.title)}</h2></div><p>${escapeHtml(category.description)}</p></div>
              <div class="activity-bank-grid">
                ${items.map((activity) => `<article class="activity-bank-card ${activity.enabled === false ? "needs-review" : ""}" data-category="${activity.category}" data-levels="${activity.levels.join(" ")}" data-search="${escapeHtml(normalized(`${activity.title} ${activity.summary} ${activity.primary}`))}">
                  <div class="activity-bank-heading"><span class="tag">${escapeHtml(activity.space)}</span>${activity.enabled === false ? `<span class="review-tag">Revisar antes de usar</span>` : ""}</div>
                  <h3>${escapeHtml(activity.title)}</h3>
                  <p>${escapeHtml(activity.summary)}</p>
                  <div class="activity-bank-meta"><span><strong>Preescolar:</strong> ${activity.levels.map(activityLevelLabel).join(" · ")}</span><span><strong>Bimestres:</strong> ${activity.bimesters.join(" · ")}</span>${activity.primary ? `<span><strong>Primaria:</strong> ${escapeHtml(activity.primary)}</span>` : ""}</div>
                </article>`).join("")}
              </div>
            </section>`;
          }).join("")}
        </div>
        <section class="empty-state panel activity-empty" hidden><h2>No hay coincidencias</h2><p>Prueba otra palabra o cambia los filtros.</p></section>
      </div>`;

    const applyFilters = () => {
      const query = normalized(state.activitySearch);
      let visibleTotal = 0;
      document.querySelectorAll(".activity-bank-card").forEach((card) => {
        const categoryMatch = state.activityCategory === "all" || card.dataset.category === state.activityCategory;
        const levelMatch = state.activityLevel === "all" || card.dataset.levels.split(" ").includes(state.activityLevel);
        const searchMatch = !query || card.dataset.search.includes(query);
        card.hidden = !(categoryMatch && levelMatch && searchMatch);
        if (!card.hidden) visibleTotal += 1;
      });
      document.querySelectorAll("[data-activity-group]").forEach((group) => {
        group.hidden = ![...group.querySelectorAll(".activity-bank-card")].some((card) => !card.hidden);
      });
      document.querySelector(".activity-empty").hidden = visibleTotal > 0;
    };
    document.querySelector("#activity-search").addEventListener("input", (event) => {
      state.activitySearch = event.target.value;
      applyFilters();
    });
    document.querySelector("#activity-category").addEventListener("change", (event) => {
      state.activityCategory = event.target.value;
      applyFilters();
    });
    document.querySelector("#activity-level").addEventListener("change", (event) => {
            ...
            .map(
              (sourceLesson, index) => {
                ...
                <article class="week-mini-card">
                  <div><span class="label">${lesson.slot === "Única" ? "Clase semanal" : `Clase ${lesson.slot}`}</span><h4>${escapeHtml(lesson.title)}</h4><p>${escapeHtml(lesson.activity)}</p>${grade.stage === "preescolar" ? `<p class="lesson-activity-options"><strong>Elige un juego:</strong> ${escapeHtml(activityNamesForLesson(grade, bimester, week, index))}</p>` : ""}${lesson.themeConnection ? `<p class="mini-theme"><strong>Tema transversal:</strong> ${escapeHtml(lesson.themeConnection)}</p>` : ""}<p class="mini-formation"><strong>Formación:</strong> ${escapeHtml(lesson.formativeObjective)}</p><p class="muted"><strong>Lenguaje:</strong> ${escapeHtml(lesson.language || "")}</p>${lessonCalendarConnections(lesson).length ? `<p class="mini-calendar"><strong>Calendario:</strong> ${lessonCalendarConnections(lesson).map((item) => item.calendar?.title || item.event).map(escapeHtml).join(" · ")}</p>` : ""}</div>
  ...
  function renderCalendar() {
    ...
    const groupRows = [...new Map((data.schedule || []).map((entry) => [entry.group, entry])).values()].map((entry) => {
      const grade = gradeById(entry.gradeId);
      const counts = [1, 2, 3, 4].map((number) => groupSessionDates(entry.group, grade, number).length);
      return { group: entry.group, grade, counts, total: counts.reduce((sum, count) => sum + count, 0) };
    });
    app.innerHTML = `
      <div class="page">
        ...
        <section class="calendar-official panel">
          <div class="section-heading"><div><span class="eyebrow">Fuente institucional</span><h2>Periodos y días académicos oficiales</h2></div><p>${escapeHtml(data.meta.calendarSource || "Calendario interno 2026-2027")}</p></div>
          <div class="calendar-period-strip">${(data.meta.academicPeriods || []).map((period) => `<article><span>Bimestre ${period.number}</span><strong>${period.totalDays} días</strong><small>${escapeHtml(period.label)}</small></article>`).join("")}</div>
        </section>

        <section class="calendar-count-table panel">
          <div class="section-heading"><div><span class="eyebrow">Horario de Música cruzado con el calendario</span><h2>Clases reales por grupo</h2></div><p>Los festivos, recesos y jornadas marcadas sin clase se descuentan automáticamente.</p></div>
          <div class="table-scroll" tabindex="0"><table><thead><tr><th>Grupo</th><th>B1</th><th>B2</th><th>B3</th><th>B4</th><th>Total</th></tr></thead><tbody>${groupRows.map((row) => `<tr><th scope="row">${escapeHtml(row.group)}</th>${row.counts.map((count) => `<td>${count}</td>`).join("")}<td><strong>${row.total}</strong></td></tr>`).join("")}</tbody></table></div>
          <p class="calendar-count-note">Las izadas de bandera y reuniones aparecen como referencias institucionales, pero no descuentan una clase completa salvo que se marquen explícitamente como “Reemplaza las clases regulares”.</p>
        </section>
    ...
    app.innerHTML = `
      <div class="page">
        ...
        <section class="resource-grid">
          ...
          <div class="resource-card download-card"><div><strong>Preescolar · PDF de consulta</strong><span>La página web contiene el calendario interno y el banco de actividades más recientes.</span></div><a class="small-button secondary" href="downloads/Curriculo_Musica_Preescolar_San_Tarsicio_2026-2027_ACTUALIZADO.pdf" download>Descargar PDF</a></div>
  ...
  function renderLessonDialog(editing = false) {
    ...
    if (editing) {
      dialogContent.innerHTML = `
        ...
        <form class="edit-form" id="lesson-edit-form">
          ...
          ${editField("themeConnection", "Conexión con el tema del bimestre", lesson.themeConnection || "")}
    ...
    const chosenActivity = grade.stage === "preescolar" ? activityById.get(selectedActivityId(key)) : null;
    const todayPlan = routine.map((moment, index) => {
      ...
      if (index === 2) instruction = chosenActivity
        ? `${chosenActivity.title}: ${chosenActivity.summary} Conectar el juego con ${lesson.language || "el objetivo musical"}.`
        : `${lesson.activity} ${grade.stage === "preescolar" ? "Elegir una de las tres opciones del Juego del día. " : ""}Interacción visual: ${lesson.visual} Luego, continuar sin pantalla.`;
    ...
    dialogContent.innerHTML = `
      <div class="lesson-title-block">
        ...
        ${lesson.themeConnection ? `<p class="lesson-theme-line"><strong>Tema transversal:</strong> ${escapeHtml(lesson.themeConnection)}</p>` : ""}
        <button class="small-button edit-button no-print" data-action="edit-curriculum">Editar contenido</button>
      </div>
      <div class="plan-grid">
        ${bimester.guidingQuestion ? `<section class="plan-block full theme-plan-block"><span class="label">Pregunta guía del bimestre</span><p><strong>${escapeHtml(bimester.guidingQuestion)}</strong></p><p>${escapeHtml(lesson.themeConnection || "")}</p></section>` : ""}
        ...
        ${renderActivityChoices(grade, bimester, week, Number(dialog.dataset.lesson), key)}
  ...
  function chooseActivity(target) {
    const key = target.dataset.key;
    const activity = activityById.get(target.dataset.activity);
    if (!key || !activity) return;
    localStorage.setItem(`st-activity-${key}`, activity.id);
    showToast(`Actividad elegida: ${activity.title}`);
    if (dialog.open) renderLessonDialog(false);
    else route();
  }
  ...
  function exportAdjustments() {
    const payload = {
      ...
      chosenActivities: curriculumStorageItems("st-activity-"),
      settings: {
        bimester: state.bimester,
        week: state.week,
        date: state.date,
  ...
  async function importAdjustments(event) {
    ...
    try {
      ...
      Object.entries(payload.chosenActivities || {}).forEach(([key, value]) => localStorage.setItem(key, value));
      if (payload.settings) {
  ...
  function route() {
    ...
    else if (view === "actividades") renderActivities();
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "choose-activity") chooseActivity(target);
